const other = [   
    {
        id: '401',
        title: 'payBack',
        category: 'apps',
        icon: 'fa-solid fa-shop',
        images: [
            './images/other/4-apps/app-401.jpeg',
        ],
        country: 'Germany',
        location: 'https://play.google.com/store/apps/details?id=de.payback.client.android&hl=en_US&gl=US',
        site: 'https://www.payback.de/',
        facebook: 'https://www.facebook.com/PAYBACK.Deutschland',
        address1: 'Theresienhöhe 12, 80339 München',
        phone: '+49089997410',
        email:'info@payback.net',
        description: 'PAYBACK is the leading bonus Aplication in Germany.',
        thingstodo: [
            'Important App you need in Germany',
            'buy',
            'colect bonus',
            'buy for free',
            'C&A',
            'Rewe',
            'DM',
        ],
        rate: 4.4,
        likes: '267,198',
        featured: true,
    },
    {
        id: '402',
        title: 'DeutschlandCard',
        category: 'apps',
        icon: 'fa-solid fa-shop',
        images: [
            './images/other/4-apps/app-402.jpeg',
        ],
        country: 'Germany',
        location: 'https://play.google.com/store/apps/details?id=de.deutschlandcard.app&hl=en_US&gl=US',
        site: 'https://www.deutschlandcard.de/',
        facebook: 'https://www.facebook.com/DeutschlandCard/',
        address1: 'Postfach 60 06 26060 Oldenburg',
        phone: '089/411 411 51',
        email:'kundenanfrage@deutschlandcard.de',
        description: 'DeutschlandCard. In Germany With your daily purchases and online shopping, you collect valuable points with your DeutschlandCard bonus card and benefit from exclusive coupon offers and attractive bonuses.',
        thingstodo: [
            'Apps in Germany',
            'buy',
            'colect bonus',
            'buy for free',
            'Netto',
            'Edeka',
        ],
        rate: 4.5,
        likes: '371,145',
        featured: true,
    },
    {
        id: '403',
        title: 'Lidl plus',
        category: 'apps',
        icon: 'fa-solid fa-shop',
        images: [
            './images/other/4-apps/app-403.jpeg',
        ],
        country: 'Germany',
        location: 'https://play.google.com/store/apps/details?id=com.lidl.eci.lidlplus&hl=en&gl=US',
        site: 'https://www.lidl.de/c/lidl-plus/s10007388',
        facebook: 'https://www.facebook.com/lidl',
        address1: 'Bonfelder Straße 2 74206 Bad Wimpfen‎',
        phone: '0800 4353361',
        email:'socialmedia@lidl.de und kontakt@lidl.de',
        description: 'Lidl Plus is your digital customer card with which you can collect new discounts every week. Scan your card at the checkout with every purchase and automatically enjoy many advantages.',
        thingstodo: [
            'buy',
            'colect bonus',
            'buy for free',
            'discounts',
            'Lidl'
        ],
        rate: 4.2,
        likes: '965,987',
        featured: true,
    },
    {
        id: '404',
        title: 'Groupon',
        category: 'apps',
        icon: 'fa-solid fa-shop',
        images: [
            './images/other/4-apps/app-404.jpeg',
        ],
        country: 'Germany',
        location: 'https://play.google.com/store/apps/details?id=com.groupon&hl=en&gl=US',
        site: 'https://www.groupon.de/',
        facebook: 'https://www.facebook.com/groupon.de/?brand_redir=182588831756639',
        address1: 'Hausvogteiplatz 2 10117, Berlin‎',
        phone: '030 311 998 66',
        email:'socialmedia@groupon.de',
        description: 'Groupon is a place where customers can discover new experiences every day and local businesses thrive. Find discounts for Restaurants, beauty and more.',
        thingstodo: [
            'Deals & Coupons',
            'Restaurants',
            'beauty',
            'discounts'
        ],
        rate: 4.2,
        likes: '999,999',
        featured: true,
    }, 
    {
        id: '405',
        title: 'Kaufda.de',
        category: 'apps',
        icon: 'fa-solid fa-shop',
        images: [
            './images/other/4-apps/app-405.jpeg',
        ],
        country: 'Germany',
        location: 'https://play.google.com/store/apps/details?id=com.bonial.kaufda&hl=en_US&gl=US',
        site: 'https://www.kaufda.de',
        facebook: 'https://www.facebook.com/kaufda/',
        address1: '‎Berlin',
        phone: '030 60989600',
        email:'info@kaufda.de',
        description: 'Kaufda.de Current offers & brochures from over 350 dealers online at a glance ⭐ Browse through brochures, save money when shopping.',
        thingstodo: [
            'Deals & Coupons',
            'super markets',
            'discounts'
        ],
        rate: 4.6,
        likes: '999,999',
        featured: true,
    },  
    {
        id: '406',
        title: 'ebay Kleinanzeigen',
        category: 'apps',
        icon: 'fa-solid fa-shop',
        images: [
            './images/other/4-apps/app-406.jpeg',
        ],
        country: 'Germany',
        location: 'https://play.google.com/store/apps/details?id=com.ebay.kleinanzeigen&hl=en&gl=US',
        site: 'https://www.ebay-kleinanzeigen.de/',
        facebook: 'https://www.facebook.com/eBayKleinanzeigen/',
        address1: 'GmbH Albert-Einstein-Ring 26 14532 Kleinmachnow',
        phone: '',
        email:'service@ebay-kleinanzeigen.de',
        description: 'ebay Kleinanzeigen is the flea market app with the best deals in your neighborhood. Post free ads with eBay Classifieds. Used or new, private or commercial - advertise now for free on Germany most visited ...',
        thingstodo: [
            'free',
            'Easy',
            'Local',
            'furniture',
            'home',
            'kitchen'
        ],
        rate: 4.7,
        likes: '999,999',
        featured: true,
    },
    {
        id: '407',
        title: 'Halal Zulal',
        category: 'apps',
        icon: 'fa-solid fa-shop',
        images: [
            './images/other/4-apps/app-407.jpeg',
        ],
        country: 'Germany',
        location: 'https://play.google.com/store/apps/details?id=com.halalzulal.mammar.halalzulal&hl=en&gl=US',
        site: 'https://www.halalzulal.com/',
        facebook: 'https://www.facebook.com/groups/1102676916432033',
        address1: '',
        phone: '',
        email:'',
        description: 'Halal Zulal. Check Halal products',
        thingstodo: [
        ],
        rate: 4.7,
        likes: '120,331',
        featured: true,
    },  
    {
        id: '408',
        title: 'rent Appartements',
        category: 'apps',
        icon: 'fa-solid fa-shop',
        images: [
            './images/other/4-apps/app-408.jpeg',
        ],
        country: 'Germany',
        location: '',
        site: '',
        facebook: '',
        address1: '',
        phone: '',
        email:'',
        description: 'Many Apps To Rent Appartements',
        thingstodo: [
            'immobilienscout24',
            'Immowelt',
            'immonet',
            'ebay Kleinanzeigen',
            'Immobilien',
            'uniplaces',
            'meinestadt',
            'wohnungsboerse',
            'wg-gesucht',
            'lieblingsmieter'
        ],
        rate: 4.7,
        likes: '190,458',
        featured: true,
    }, 
    {
        id: '409',
        title: 'Transportations ',
        category: 'apps',
        icon: 'fa-solid fa-shop',
        images: [
            './images/other/4-apps/app-409.jpeg',
        ],
        country: 'Germany',
        location: '',
        site: '',
        facebook: '',
        address1: '',
        phone: '',
        email:'',
        description: 'Transportations &Tickets &Navigations',
        thingstodo: [
            'MVG',
            'MVV',
            'DB navigator',
            'Googel maps',
        ],
        rate: 4.7,
        likes: '496,234',
        featured: true,
    },
    {
        id: '410',
        title: 'Online shopping',
        category: 'apps',
        icon: 'fa-solid fa-shop',
        images: [
            './images/other/4-apps/app-410.jpeg',
        ],
        country: 'Germany',
        location: '',
        site: '',
        facebook: '',
        address1: '',
        phone: '',
        email:'',
        description: 'Online shopping app in Germany',
        thingstodo: [
            'Amazon',
            'Zalando',
            'Shein',
            'Idealo',
            'Lidl',
            'Real',
            'OTTO.DE',
            'MIRAPODO',
            'modanisa',
        ],
        rate: 4.7,
        likes: '977,248',
        featured: true,
    },  
    {
        id: '411',
        title: 'Doctor Appointment',
        category: 'apps',
        icon: 'fa-solid fa-shop',
        images: [
            './images/other/4-apps/app-411.jpeg',
        ],
        country: 'Germany',
        location: '',
        site: '',
        facebook: '',
        address1: '',
        phone: '',
        email:'',
        description: 'Online shopping app in Germany',
        thingstodo: [
            'Doctolib',
            'jameda',
            'sanego',
        ],
        rate: 4.7,
        likes: '996,483',
        featured: true,
    }, 
    {
        id: '412',
        title: 'Food delivery -Restaurants',
        category: 'apps',
        icon: 'fa-solid fa-shop',
        images: [
            './images/other/4-apps/app-412.jpeg',
        ],
        country: 'Germany',
        location: '',
        site: '',
        facebook: '',
        address1: '',
        phone: '',
        email:'',
        description: 'Food delivery -Restaurants',
        thingstodo: [
            'Lieferando',
            'wolt',
            'Uber eats',
            'schnellliefer',
        ],
        rate: 4.7,
        likes: '996,483',
        featured: true,
    },  
    {
        id: '413',
        title: 'order halal food in Germany',
        category: 'apps',
        icon: 'fa-solid fa-shop',
        images: [
            './images/other/4-apps/app-413.jpeg',
        ],
        country: 'Germany',
        location: '',
        site: '',
        facebook: '',
        address1: '',
        phone: '',
        email:'',
        description: 'order halal food in Germany',
        thingstodo: [
            'GetHalal',
            'yam egyptian food',
            'almasry market',
            'mozzaik',
        ],
        rate: 4.7,
        likes: '996,483',
        featured: true,
    }, 
    {
        id: '414',
        title: 'TV Entertainment',
        category: 'apps',
        icon: 'fa-solid fa-shop',
        images: [
            './images/other/4-apps/app-414.jpeg',
        ],
        country: 'Germany',
        location: '',
        site: '',
        facebook: '',
        address1: '',
        phone: '',
        email:'',
        description: 'TV Entertainment',
        thingstodo: [
            'Amazon Prime',
            'Shahid VIP',
            'WatchIt',
            'Netflix',
            'IPTV'
        ],
        rate: 4.7,
        likes: '996,483',
        featured: true,
    }, 
    {
        id: '415',
        title: 'Payment & Banks',
        category: 'apps',
        icon: 'fa-solid fa-shop',
        images: [
            './images/other/4-apps/app-415.jpeg',
        ],
        country: 'Germany',
        location: '',
        site: '',
        facebook: '',
        address1: '',
        phone: '',
        email:'',
        description: 'Payment & Banks Acounts, Cards',
        thingstodo: [
            'Paypal',
            'N26',
            'Vivid',
        ],
        rate: 4.7,
        likes: '996,483',
        featured: true,
    },   
    {
        id: '416',
        title: 'Order Groceries',
        category: 'apps',
        icon: 'fa-solid fa-shop',
        images: [
            './images/other/4-apps/app-416.jpeg',
        ],
        country: 'Germany',
        location: '',
        site: '',
        facebook: '',
        address1: '',
        phone: '',
        email:'',
        description: 'Order Groceries from Super Markets',
        thingstodo: [
            'Flink',
            'REWE',
            'Amazon Fresh',
            'knuspr.de',
            'Bringmeister'
        ],
        rate: 4.7,
        likes: '996,483',
        featured: true,
    },  
    {
        id: '417',
        title: 'Order Medicine',
        category: 'apps',
        icon: 'fa-solid fa-shop',
        images: [
            './images/other/4-apps/app-417.jpeg',
        ],
        country: 'Germany',
        location: '',
        site: '',
        facebook: '',
        address1: '',
        phone: '',
        email:'',
        description: 'Order Medicine from pharmacy',
        thingstodo: [
            'Mayd',
            'Shop Apotheke',
            'Medpex',
        ],
        rate: 4.7,
        likes: '996,483',
        featured: true,
    }, 
    {
        id: '418',
        title: 'Car & Taxi & Bike services',
        category: 'apps',
        icon: 'fa-solid fa-shop',
        images: [
            './images/other/4-apps/app-418.jpeg',
        ],
        country: 'Germany',
        location: '',
        site: '',
        facebook: '',
        address1: '',
        phone: '',
        email:'',
        description: 'Car services like : Car Rental , Car Sharing , Car Parking payment , Easy teacher ',
        thingstodo: [
            'Sixt (Car Rental) ',
            'Share Now (Car Rental) ',
            'BlaBlaCar (Car Sharing) ',
            'HandyParken München (Car Parking payment)',
            'Easy teacher (Theoretical driver license)',
            'Freenow (Taxi  , Scooter  , Uber renting ) ',
            'MVGO (Bike Rental )'
        ],
        rate: 4.7,
        likes: '996,483',
        featured: true,
    }, 
    {
        id: '419',
        title: 'Money Transfers',
        category: 'apps',
        icon: 'fa-solid fa-shop',
        images: [
            './images/other/4-apps/app-419.jpeg',
        ],
        country: 'Germany',
        location: '',
        site: '',
        facebook: '',
        address1: '',
        phone: '',
        email:'',
        description: 'Money Transfers ',
        thingstodo: [
            'Transferwise',
            'MoneyGram',
            'Ria',
            'western union',
        ],
        rate: 4.7,
        likes: '996,483',
        featured: true,
    },      
    {
        id: '420',
        title: 'Tax Declaration (Steuererklärung)',
        category: 'apps',
        icon: 'fa-solid fa-shop',
        images: [
            './images/other/4-apps/app-420.jpeg',
        ],
        country: 'Germany',
        location: '',
        site: '',
        facebook: '',
        address1: '',
        phone: '',
        email:'',
        description: 'Tax Declaration (Steuererklärung) ',
        thingstodo: [
            'Steuergo',
            'TaxFix ',
        ],
        rate: 4.7,
        likes: '996,483',
        featured: true,
    },  
    {
        id: '421',
        title: 'Neighbours Ask to Help',
        category: 'apps',
        icon: 'fa-solid fa-shop',
        images: [
            './images/other/4-apps/app-421.jpeg',
        ],
        country: 'Germany',
        location: '',
        site: '',
        facebook: '',
        address1: '',
        phone: '',
        email:'',
        description: 'Neighbours Ask to Help',
        thingstodo: [
            'Nebenen',
        ],
        rate: 4.7,
        likes: '996,483',
        featured: true,
    }, 
    {
        id: '422',
        title: 'Find a Job app',
        category: 'apps',
        icon: 'fa-solid fa-shop',
        images: [
            './images/other/4-apps/app-422.jpeg',
        ],
        country: 'Germany',
        location: '',
        site: '',
        facebook: '',
        address1: '',
        phone: '',
        email:'',
        description: 'Find a Job app',
        thingstodo: [
            'zenjob ',
            'linkedIn',
            'xing',
            'stepStone',
            'glassdoor',
            'indeed',
            'stack overflow',
            'moberries',
            'joinimagine',
            'monster.de',
            'helpling.de',
            'freelancer.com',
            'upwork (freelance)',
            'fiverr (freelance)'
        ],
        rate: 4.7,
        likes: '996,483',
        featured: true,
    },   
    {
        id: '423',
        title: 'Weather',
        category: 'apps',
        icon: 'fa-solid fa-shop',
        images: [
            './images/other/4-apps/app-423.jpeg',
        ],
        country: 'Germany',
        location: '',
        site: '',
        facebook: '',
        address1: '',
        phone: '',
        email:'',
        description: 'Weather',
        thingstodo: [
            'Siri Das Wetter',
            'Windy ',
        ],
        rate: 4.7,
        likes: '996,483',
        featured: true,
    },
    {
        id: '424',
        title: 'App for Immigrants',
        category: 'apps',
        icon: 'fa-solid fa-shop',
        images: [
            './images/other/4-apps/app-424.jpeg',
        ],
        country: 'Germany',
        location: '',
        site: '',
        facebook: '',
        address1: '',
        phone: '',
        email:'',
        description: 'App Design with us a digital integration platform for your region. Multilingual & Open Source. Help refugees and migrants in your region with the mulitlingual migration. Integreat is a digital integration platform consisting of an offline usable app, website and PDF brochure. Collect regionally relevant information in general for immigrants or specifically for skilled workers or refugees on an easy-to-use digital platform',
        thingstodo: [
            'integreat',
        ],
        rate: 4.7,
        likes: '996,483',
        featured: true,
    }, 
    {
        id: '425',
        title: 'App for News',
        category: 'apps',
        icon: 'fa-solid fa-shop',
        images: [
            './images/other/4-apps/app-425.jpeg',
        ],
        country: 'Germany',
        location: '',
        site: '',
        facebook: '',
        address1: '',
        phone: '',
        email:'',
        description: 'News',
        thingstodo: [
            'Tagesschau ',
            'Nachrichten '
        ],
        rate: 4.7,
        likes: '996,483',
        featured: true,
    }, 
    {
        id: '426',
        title: 'Arabic & German translate',
        category: 'apps',
        icon: 'fa-solid fa-shop',
        images: [
            './images/other/4-apps/app-426.jpeg',
        ],
        country: 'Germany',
        location: '',
        site: '',
        facebook: '',
        address1: '',
        phone: '',
        email:'',
        description: 'Arabic & German translate',
        thingstodo: [
            'Translation yandex ',
            'Qtranslate ',
            'Arabdict '
        ],
        rate: 4.7,
        likes: '996,483',
        featured: true,
    },
    {
        id: '427',
        title: 'Learn German with teacher',
        category: 'apps',
        icon: 'fa-solid fa-shop',
        images: [
            './images/other/4-apps/app-427.jpeg',
        ],
        country: 'Germany',
        location: '',
        site: '',
        facebook: '',
        address1: '',
        phone: '',
        email:'',
        description: 'Learn German with teacher',
        thingstodo: [
            'Italki ',
            'Preply ',
            'Chatterbug '
        ],
        rate: 4.7,
        likes: '996,483',
        featured: true,
    },  
    {
        id: '428',
        title: 'Prayer time & Qibla',
        category: 'apps',
        icon: 'fa-solid fa-shop',
        images: [
            './images/other/4-apps/app-428.jpeg',
        ],
        country: 'Germany',
        location: '',
        site: '',
        facebook: '',
        address1: '',
        phone: '',
        email:'',
        description: 'Prayer time & Qibla',
        thingstodo: [
            'Muslim pro ',
            'Azan Munich ',
        ],
        rate: 4.7,
        likes: '996,483',
        featured: true,
    }, 
    {
        id: '429',
        title: 'Web app all about Munich City',
        category: 'apps',
        icon: 'fa-solid fa-shop',
        images: [
            './images/other/4-apps/app-429.jpeg',
        ],
        country: 'Germany',
        location: '',
        site: '',
        facebook: '',
        address1: '',
        phone: '',
        email:'',
        description: 'Web app all about Munich City',
        thingstodo: [
            'München.de',
            'explory24.com',
        ],
        rate: 4.7,
        likes: '996,483',
        featured: true,
    },  
    {
        id: '430',
        title: 'McDonald`s Coupons & Deals',
        category: 'apps',
        icon: 'fa-solid fa-shop',
        images: [
            './images/other/4-apps/app-430.jpeg',
        ],
        country: 'Germany',
        location: '',
        site: '',
        facebook: '',
        address1: '',
        phone: '',
        email:'',
        description: 'McDonald`s Coupons & Deals Near You',
        thingstodo: [
            'mcdonalds',
        ],
        rate: 4.7,
        likes: '996,483',
        featured: true,
    },  
    {
        id: '430',
        title: 'McDonald`s Coupons & Deals',
        category: 'apps',
        icon: 'fa-solid fa-shop',
        images: [
            './images/other/4-apps/app-430.jpeg',
        ],
        country: 'Germany',
        location: '',
        site: '',
        facebook: '',
        address1: '',
        phone: '',
        email:'',
        description: 'McDonald`s Coupons & Deals Near You',
        thingstodo: [
            'mcdonalds',
        ],
        rate: 4.7,
        likes: '996,483',
        featured: true,
    },  
    {
        id: '431',
        title: 'meta search engine for flights',
        category: 'apps',
        icon: 'fa-solid fa-shop',
        images: [
            './images/other/4-apps/app-431.jpeg',
        ],
        country: 'Germany',
        location: '',
        site: '',
        facebook: '',
        address1: '',
        phone: '',
        email:'',
        description: 'meta search engine for flights and travel agency, The company lets people research and book travel options for their trips, including flights, hotels and car hire. Wikipedia',
        thingstodo: [
            'skyscanner',
            'google flights',
            'momondo',
            'Opodo',
            'swoodoo',
            'Flugladen',
        ],
        rate: 4.7,
        likes: '996,483',
        featured: true,
    },   
    {
        id: '432',
        title: 'cheap flights',
        category: 'apps',
        icon: 'fa-solid fa-shop',
        images: [
            './images/other/4-apps/app-432.jpeg',
        ],
        country: 'Germany',
        location: '',
        site: '',
        facebook: '',
        address1: '',
        phone: '',
        email:'',
        description: 'Cheap Flights, low-cost and charter airline.',
        thingstodo: [
            'flyegypt',
            'Wizzair',
            'Ryanair',
            'EasyJet',
            'Condor',
            'Air Cairo',
        ],
        rate: 4.7,
        likes: '996,483',
        featured: true,
    },
    {
        id: '433',
        title: 'check24',
        category: 'apps',
        icon: 'fa-solid fa-shop',
        images: [
            './images/other/4-apps/app-433.jpeg',
        ],
        country: 'Germany',
        location: '',
        site: '',
        facebook: '',
        address1: '',
        phone: '',
        email:'',
        description: 'Comparison portal: insurance, loans, energy, DSL, travel , hotels, mobile lines.',
        thingstodo: [
            'insurance',
            'energy',
            'DSL',
            'travel',
            'hotels',
            'mobile lines',
        ],
        rate: 4.7,
        likes: '996,483',
        featured: true,
    },  
    {
        id: '434',
        title: 'Lodging Reservations',
        category: 'apps',
        icon: 'fa-solid fa-shop',
        images: [
            './images/other/4-apps/app-434.jpeg',
        ],
        country: 'Germany',
        location: '',
        site: '',
        facebook: '',
        address1: '',
        phone: '',
        email:'',
        description: 'Lodging Reservations.',
        thingstodo: [
            'Booking',
            'Airbnb',
        ],
        rate: 4.7,
        likes: '996,483',
        featured: true,
    },    
    {
        id: '101',
        title: 'DAR e.V. Zahr Elyasmin',
        category: 'arabic schools',
        icon: 'fa-solid fa-shop',
        images: [
            './images/other/1-arabicschools/arabicschool-101.jpeg',
        ],
        country: 'Germany',
        location: 'https://goo.gl/maps/54hcNGYRcybzZGss6',
        site: 'http://www.dar-ev.de/',
        facebook: 'https://www.facebook.com/darev2012',
        address1: 'Elisabethpl. 4, 80796 München ',
        address2:'Astrid-Lindgren-Straße 1, 81829 München',
        termin: 'https://app.dar-ev.de/de',
        description: 'We seek to create a conscious, integrated and positively influential Muslim generation in German society',
        thingstodo: [
            'learn arabic language',
            'activites',
            'camps',
            'trips',
            'sports day'
        ],
        rate: 4.6,
        likes: '2,391',
        featured: true,
    },  
    {
        id: '201',
        title: 'Islamic Centre of Munich',
        category: 'mosques',
        icon: 'fa-solid fa-shop',
        images: [
            './images/other/2-mosques/mosque-201.jpeg',
        ],
        country: 'Germany',
        location: 'https://goo.gl/maps/yVok6t8p3aw8TySP7',
        site: 'https://www.islamisches-zentrum-muenchen.de/',
        facebook: 'https://www.facebook.com/islamischeszentrummuenchen',
        address1: 'Wallnerstraße 1, 80939 München',
        phone: '+4989325061',
        description: 'The Islamic Center Munich wants to help the Muslims living here to practice their religion, to deepen their awareness of faith and their willingness to serve their fellow human beings, but also to contribute to a better understanding of Islam and Muslims. As one of the outstanding Muslim institutions in Germany, the Islamic Center has an impact far beyond the Munich area.',
        thingstodo: [
            'pray',
            'Eid prayer',
            'zakat Elmal',
            'donations',
            'arabic school'
        ],
        rate: 4.7,
        likes: '3,264',
        featured: true,
    },
    {
        id: '301',
        title: '3esh & mal7',
        category: 'food homemade',
        icon: 'fa-solid fa-shop',
        images: [
            './images/other/3-foodhomemade/foodhomemade-301.jpeg',
        ],
        country: 'Germany',
        location: '',
        site: '',
        facebook: 'https://www.facebook.com/groups/376823237021474',
        address1: '',
        phone: '',
        description: 'Egyptian Bakery in Munich 🇪🇬.',
        thingstodo: [
            'Meshalteh Pie',
            'baladi bread',
            'boksomat',
            'fool',
            'ka7k',
            '3`orayba',
            'ka3b el3`azal',
            'koras',
            'basksweet',
            'sable',
            'Lancashire',
        ],
        rate: 4.4,
        likes: '367',
        price: 8,
        featured: true,
    },
    {
        id: '302',
        title: 'Nesto Torten',
        category: 'food homemade',
        icon: 'fa-solid fa-shop',
        images: [
            './images/other/3-foodhomemade/foodhomemade-302.jpeg',
        ],
        country: 'Germany',
        location: '',
        site: '',
        facebook: 'https://www.facebook.com/Nestotorten',
        address1: '',
        phone: '',
        description: '3D cake passion.',
        thingstodo: [
            'Cake Sticks',
            'cup cake',
            '3D cake',
        ],
        rate: 4.8,
        likes: '1,144',
        prce: 60,
        featured: true,
    },     
    {
        id: '601',
        title: 'fit star',
        category: 'gym',
        icon: 'fa-solid fa-shop',
        images: [
            './images/other/6-gym/gym-601.jpeg',
        ],
        country: 'Germany',
        location: 'https://goo.gl/maps/C1T5QtkbR49EFZiA6',
        site: 'https://www.fit-star.de/fitnessstudio/muenchen-neuhausen/',
        facebook: 'https://www.facebook.com/FITSTAR.Muenchen/',
        address1: 'Dachauer Str. 92, 80335 München',
        phone: '+498954726606',
        email:'',
        description: 'Fitness training of the highest quality! Devices from the world market leader, open 24/7, course program, drinks flat rate, and much more from 19.90 euros.',
        thingstodo: [
            'open 24/7',
            'parking',
            'Best price',
        ],
        rate: 4.1,
        likes: '9,369',
        price: 19.90,
        featured: true,
    },   
    {
        id: '701',
        title: 'Maxxi Clean',
        category: 'products',
        icon: 'fa-solid fa-shop',
        images: [
            './images/other/7-products/product-701.jpeg',
        ],
        country: 'Germany',
        location: '',
        site: 'https://www.amazon.de/dp/B0157B9372?ref_=cm_sw_r_cp_ud_dp_X0FJNJ7X15ZZ48REBPJ5',
        facebook: '',
        address1: '',
        phone: '',
        description: 'Maxxi Clean Gel Paste Oven Cleaner, 500 ml, With Oven and Grill Cleaning Brush, Dissolves the Most Stubborn Encrustations, Cleans By Itself Without Heating Up, No Odours + Bonus Maxxi Clean Antibac Fleece ',
        thingstodo: [
            'amazon',
            'Oven Cleaner',
            'best seller',
        ],
        rate: 4.4,
        likes: '18,455',
        price: 14,
        featured: true,
    },   
];

export default other;